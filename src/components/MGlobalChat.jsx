  ...
  return (
    <div className="flex flex-col w-full h-full bg-white overflow-hidden">
      ...
      {latestSuperAdmin.length > 0 && (
        <div className="bg-gradient-to-r from-yellow-100 to-orange-100 border-b-2 border-yellow-400 p-3">
          {latestSuperAdmin.map((msg) => (
            <div
              key={msg.id}
              className="rounded-xl px-4 py-3 bg-white/80 shadow-md border border-yellow-300"
            >
              <span className="font-black text-red-600 text-sm flex items-center gap-2">
                <Megaphone className="w-4 h-4" />
                {msg.username}
              </span>
              <div className="text-gray-800 break-words whitespace-pre-wrap mt-1 font-medium text-sm">
                {msg.message}
              </div>
            </div>
          ))}
        ...
        {normalMessages.map((msg, i) => {
          ...
          return (
            ...
            >
              ...
              >
                {showUser && !isMe && (
                  <span className="font-black text-pink-500 block text-xs mb-1 flex items-center gap-1">
                    {msg.username}
                    {msg.is_admin && <Crown className="w-3 h-3 text-yellow-500" />}
                  </span>
                )}
      ...
      {/* Sticky Input */}
      <div className="sticky bottom-0 flex gap-2 border-t-2 border-gray-100 bg-white p-3">
        <input
          className="flex-1 p-3 text-sm outline-none border-2 border-gray-200 rounded-xl focus:border-pink-400 focus:ring-2 focus:ring-pink-200 transition bg-gray-50"
          value={newMessage}
        ...
        <button
          onClick={sendMessage}
          className="px-5 py-3 bg-gradient-to-b from-pink-400 to-pink-500 text-white rounded-xl hover:from-pink-500 hover:to-pink-600 active:translate-y-0.5 transition-all shadow-[0_3px_0_#be185d] font-bold flex items-center justify-center"
        >
